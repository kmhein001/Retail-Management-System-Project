...
if (isset($_GET['id']) && is_numeric($_GET['id']) && $_GET['id'] > 0) {
    ...
    $stmt = $conn->prepare("SELECT ImageFile FROM Subcategory WHERE SubcategoryID = ?");
    if ($stmt === false) {
    ...
    if ($result->num_rows > 0) {
        ...
        $delete_stmt = $conn->prepare("DELETE FROM Subcategory WHERE SubcategoryID = ?");
        ...
        if ($delete_stmt->execute()) {
            header("Location: subcategory_list.php?success=Subcategory deleted successfully.");
            exit();
        } else {
...
" . $delete_stmt->error);
        ...
        header("Location: subcategory_list.php?error=Record not found for ID: " . $id);
        exit();
    }
    ...
    header("Location: subcategory_list.php?error=No ID provided.");
    exit();
}