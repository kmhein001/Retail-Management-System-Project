...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_FILES['ImageFile']) && $_FILES['ImageFile']['error'] === UPLOAD_ERR_OK) {
        ...
        if ($file_extension === 'svg') {
            $target_directory = '../assets/img/brand/svg/';
        } elseif ($file_extension === 'png') {
            $target_directory = '../assets/img/brand/png/';
        } else {
    ...
    $stmt = $conn->prepare("INSERT INTO Brands (BrandName, ImageFile, CountryName, CreatedBy, Status)
                            VALUES ( ?, ?, ?, ?, ?)");
    if (!$stmt) {
    ...
    // Bind parameters
    $stmt->bind_param("sssss", $name, $img,  $ccode, $cby, $status);
