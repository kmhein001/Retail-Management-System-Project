...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_FILES['ImageFile']) && $_FILES['ImageFile']['error'] === UPLOAD_ERR_OK) {
        ...
        if ($file_extension === 'svg') {
            $target_directory = '../assets/img/category/svg/';
        } elseif ($file_extension === 'png') {
            $target_directory = '../assets/img/category/png/';
        } else {
    ...
    $stmt = $conn->prepare("INSERT INTO Category (CategoryName, CategoryCode, ImageFile, Description, CreatedBy, Status)
                            VALUES ( ?, ?, ?, ?, ?, ?)");
    if (!$stmt) {
    ...
    // Bind parameters
    $stmt->bind_param("ssssss", $name, $ccode, $img, $desc, $cby, $status);
