...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "UPDATE Category SET CategoryName = ?, CategoryCode = ?, Description = ?, CreatedBy = ?, Status = ?";
    if ($img) {
        $query .= ", ImageFile = ?";
    }
    ...
    if ($img) {
        $stmt->bind_param("sssssssi", $name, $ccode, $desc, $cby, $status, $img, $id);
    } else {
        $stmt->bind_param("ssssssi", $name, $ccode, $desc, $cby, $status, $id);
    }

    if ($stmt->execute()) {
        header("Location: category_list.php?success=category updated successfully.");
    } else {
