...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['delete_selected'])) {
    if (!empty($_POST['selected_ids'])) {
        ...
        $stmt = $conn->prepare("SELECT ImageFile FROM Products WHERE ProductID IN ($placeholders)");
        ...
        $delete_stmt = $conn->prepare("DELETE FROM Products WHERE ProductID IN ($placeholders)");
        $delete_stmt->bind_param(str_repeat('i', count($selected_ids)), ...$selected_ids);

        if ($delete_stmt->execute()) {
            header("Location: product_list.php?success=Selected countries deleted successfully.");
        } else {
        ...
        header("Location: product_list.php?error=No countries selected.");
    }
} else {
