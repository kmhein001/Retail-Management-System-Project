...
if (isset($_GET['id']) && is_numeric($_GET['id']) && $_GET['id'] > 0) {
    ...
    $stmt = $conn->prepare("SELECT ImageFile From Products WHERE ProductID = ?");
    if ($stmt === false) {
    ...
    if ($result->num_rows > 0) {
        ...
        if ($delete_stmt->execute()) {
            header("Location: product_list.php?success=brand deleted successfully.");
            exit();
        } else {
...
" . $delete_stmt->error);
        ...
        header("Location: product_list.php?error=Record not found for ID: " . $id);
        exit();
    }
    ...
    header("Location: product_list.php?error=No ID provided.");
    exit();
}